...
streamdeck-mcp ships with an **Agent Skill** that teaches Claude how to design, theme, and author complete Stream Deck layouts using the MCP tools below. With the skill loaded, Claude can one-shot an authored deck from a high-level prompt — "give me a hello-kitty-themed Twitch deck with Hue light controls", "build a dev deck for this repo in Nordic colors", "build me a Slack control board with channel jumps, status toggles, and unread dials" — including palette planning, integration discovery, consistent icon generation, and shell-script wiring.
